...
import { SpinningBox } from '@/components/SpiningBox'
THREE.ColorManagement.legacyMode = false

const backgroundImages = [
  './src/assets/views/tv.jpg',
  './src/assets/views/next.jpg',
  './src/assets/views/css.webp',
  './src/assets/views/angular.png',
  './src/assets/views/js.png',
  './src/assets/views/node.webp',
]
...
function ScreenText({ x = 0, y = 1.2, ...props }) {
  ...
  useEffect(() => {
    ...
    const imagePath = backgroundImages[imageIndex];
  ...
  useEffect(() => {
    function handleResize() {
      ...
      setPlaneSize({ width: aspect * 6, height: 4 });
    }
  ...
  return (
    <Screen {...props}>
      ...
      {backgroundTexture && (
        <mesh  position={[0, 1, -1]} >
          <planeBufferGeometry args={[planeSize.width, planeSize.height]} />
          <meshBasicMaterial map={backgroundTexture} />
        </mesh>
      )}
      <ambientLight intensity={0.5} />
      <directionalLight position={[10, 10, 5]} />
      <Text  position={[x, y, 0]} ref={textRef} fontWeight={"bold"} fontSize={1.5} letterSpacing={-0.1} color={imageIndex % 2 === 0 ? 'black' : '#35c19f'}>
...
/*ScreenInteractive Renders a monitor with a spinning box */
